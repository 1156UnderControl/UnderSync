...
function normalizedCode(value: string): string {
  const code = value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase().replace(/[^A-Z0-9]+/g, "_").replace(/^_|_$/g, "").slice(0, 40);
  if (code.length < 2) throw new Error("Enter a status code with at least two letters or numbers.");
  return code;
}
...
export const get = query({
  args: {},
  returns: v.object({ types: v.array(typeValidator), statuses: v.array(statusValidator), fields: v.array(fieldValidator) }),
  handler: async (ctx) => {
    await requireAdmin(ctx);
    const [types, statuses] = await Promise.all([
      ctx.db.query("cotsTypes").order("asc").take(100),
      ctx.db.query("cotsStatuses").order("asc").take(100),
    ]);
    const fields = [];
...
export const addType = mutation({
  args: { name: v.string(), slug: v.string(), icon: v.string(), sortOrder: v.number() }, returns: v.id("cotsTypes"),
  handler: async (ctx, args) => {
    await requireAdmin(ctx);
    const name = args.name.trim(); const slug = normalizedKey(args.slug || name); const icon = args.icon.trim();
    if (name.length < 2 || name.length > 100 || icon.length < 1 || icon.length > 500) throw new Error("Enter a valid COTS type name and icon/image URL.");
    ...
    return await ctx.db.insert("cotsTypes", { name, slug, icon, sortOrder: validOrder(args.sortOrder), active: true });
  },
});

export const updateType = mutation({
  args: { id: v.id("cotsTypes"), name: v.string(), icon: v.string(), sortOrder: v.number(), active: v.boolean() }, returns: v.null(),
  handler: async (ctx, args) => {
    await requireAdmin(ctx); const name = args.name.trim(); const icon = args.icon.trim();
    if (name.length < 2 || name.length > 100 || icon.length < 1 || icon.length > 500) throw new Error("Enter a valid COTS type name and icon/image URL.");
    await ctx.db.patch("cotsTypes", args.id, { name, icon, sortOrder: validOrder(args.sortOrder), active: args.active }); return null;
  },
});
...
export const deleteType = mutation({
  ...
  handler: async (ctx, args) => {
    await requireAdmin(ctx);
    if (await ctx.db.query("cotsItems").withIndex("by_cotsTypeId", (q) => q.eq("cotsTypeId", args.id)).first()) {
      throw new Error("This type contains COTS items. Disable it instead.");
    }
    ...
    for (const field of fields) await ctx.db.delete("cotsFieldDefinitions", field._id);
...
export const updateStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (name.length < 2 || name.length > 100) throw new Error("Enter a valid quantity status name.");
    await ctx.db.patch("cotsStatuses", args.id, { name, sortOrder: validOrder(args.sortOrder), active: args.active }); return null;
  },
});
...
export const deleteStatus = mutation({
  ...
  handler: async (ctx, args) => {
    await requireAdmin(ctx);
    if (await ctx.db.query("cotsItemQuantities").withIndex("by_statusId", (q) => q.eq("statusId", args.id)).first()) {
      throw new Error("This status is used by COTS inventory. Disable it instead.");
    }
    await ctx.db.delete("cotsStatuses", args.id); return null;
  },
});
...
export const deleteField = mutation({
  ...
  handler: async (ctx, args) => {
    await requireAdmin(ctx);
    const values = await ctx.db.query("cotsItemFieldValues").withIndex("by_fieldDefinitionId", (q) => q.eq("fieldDefinitionId", args.id)).take(500);
