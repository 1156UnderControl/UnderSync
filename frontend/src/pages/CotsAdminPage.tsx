    ...
    <section className="panel span-all"><div className="panel-heading"><div><p className="eyebrow">Type-specific form</p><h2>Custom questions</h2></div></div>
      ...
      {!selectedType ? <p className="empty-state">Add a COTS type before configuring questions.</p> : <>
        <form className="cots-admin-add cots-field-add" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget);
          void run(() => addField({ cotsTypeId: selectedType.id, label: String(form.get("label")), key: String(form.get("key")), fieldType: String(form.get("fieldType")) as "STRING" | "BOOLEAN", sortOrder: Number(form.get("sortOrder")) }), "Question added.").then(() => event.currentTarget.reset()); }}>
          <input name="label" placeholder="Number of teeth" required /><input name="key" placeholder="number-of-teeth" required />
          <select name="fieldType" defaultValue="STRING" aria-label="Answer type"><option value="STRING">Text</option><option value="BOOLEAN">True / false</option></select>
          <input name="sortOrder" type="number" min={0} defaultValue={(selectedFields.at(-1)?.sortOrder ?? 0) + 10} required /><button className="button button-primary">Add question</button></form>
        <div className="cots-question-list">{selectedFields.length === 0 ? <p className="empty-state">No custom questions for {selectedType.name}.</p> : selectedFields.map((field, index) => <form key={field.id} onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget);
          void run(() => updateField({ id: field.id, label: String(form.get("label")), fieldType: String(form.get("fieldType")) as "STRING" | "BOOLEAN", sortOrder: Number(form.get("sortOrder")), active: field.active }), "Question updated."); }}>
          <span className="question-position">{index + 1}</span><input name="label" defaultValue={field.label} required /><span className="code-badge">{field.key}</span>
          <select name="fieldType" defaultValue={field.fieldType} aria-label="Answer type"><option value="STRING">Text</option><option value="BOOLEAN">True / false</option></select>
          <input className="order-input" name="sortOrder" type="number" min={0} defaultValue={field.sortOrder} aria-label="Question order" required />
          <div className="row-actions"><button className="button button-small button-primary">Save</button><button className="button button-small" type="button" onClick={() => void run(() => updateField({ id: field.id, label: field.label, fieldType: field.fieldType, sortOrder: field.sortOrder, active: !field.active }), field.active ? "Question disabled." : "Question enabled.")}>{field.active ? "Disable" : "Enable"}</button>
